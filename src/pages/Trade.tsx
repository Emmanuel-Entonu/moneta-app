...
function OrderReceipt({ receipt, pacAccountId, onNewTrade, onViewPortfolio }: {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: '#070e1a', zIndex: 200, display: 'flex', flexDirection: 'column', paddingTop: 'env(safe-area-inset-top,0px)', paddingBottom: 'env(safe-area-inset-bottom,0px)', animation: 'fadeIn 0.25s ease both' }}>
      <div style={{ flex: 1, overflowY: 'auto', display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '52px 20px 24px' }}>

        {/* Success icon — always green checkmark */}
        <div style={{ width: 88, height: 88, borderRadius: '50%', background: 'linear-gradient(135deg,#059669,#047857)', display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 22, boxShadow: '0 0 56px rgba(5,150,105,0.45)', animation: 'popIn 0.5s cubic-bezier(0.34,1.56,0.64,1) both' }}>
        ...
        <p style={{ fontSize: 13, fontWeight: 700, color: 'rgba(255,255,255,0.4)', letterSpacing: 0.5, textTransform: 'uppercase', marginBottom: 6 }}>
          {isBuy ? 'Buy Order Placed' : 'Sell Order Placed'}
        </p>
        <p style={{ fontSize: 36, fontWeight: 900, color: '#fff', letterSpacing: -1, marginBottom: 4, lineHeight: 1 }}>
          {fmt(receipt.total)}
        </p>
        <p style={{ fontSize: 13, color: 'rgba(255,255,255,0.35)', fontWeight: 500, marginBottom: 32 }}>
          {receipt.qty.toLocaleString()} shares · {receipt.symbol}
        </p>

        {/* Order detail card */}
        <div style={{ width: '100%', background: '#0e1c2f', borderRadius: 20, border: '1px solid rgba(255,255,255,0.07)', overflow: 'hidden', marginBottom: 12 }}>
          {([
            ['Stock',             receipt.name],
            ...
            receipt.validation ? ['Fees', fmt(receipt.validation.commission + receipt.validation.fees)] : null,
            ['Total',             fmt(receipt.total)],
          ] as ([string, string] | null)[]).filter((x): x is [string, string] => x !== null).map(([label, value], i, arr) => (
            <div key={label} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '13px 18px', borderBottom: i < arr.length - 1 ? '1px solid rgba(255,255,255,0.05)' : 'none' }}>
              <span style={{ fontSize: 13, color: 'rgba(255,255,255,0.38)', fontWeight: 500 }}>{label}</span>
              <span style={{ fontSize: 13, fontWeight: 800, color: label === 'Total' ? '#34d399' : label === 'Side' ? (isBuy ? '#34d399' : '#f87171') : '#fff' }}>{value}</span>
            </div>
          ))}
        </div>

        {/* Status + time card */}
        <div style={{ width: '100%', background: '#0e1c2f', borderRadius: 16, border: '1px solid rgba(255,255,255,0.07)', overflow: 'hidden', marginBottom: 12 }}>
          {([
            ...
            <div key={label} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px 18px', borderBottom: i < arr.length - 1 ? '1px solid rgba(255,255,255,0.05)' : 'none' }}>
              <span style={{ fontSize: 12, color: 'rgba(255,255,255,0.32)', fontWeight: 500 }}>{label}</span>
              <span style={{ fontSize: 12, fontWeight: 700, color: label === 'Status' ? '#34d399' : 'rgba(255,255,255,0.6)' }}>{value}</span>
            </div>
          ))}
        </div>
          ...
          <div>
            ...
            {balStatus === 'polling' ? (
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                ...
                <span style={{ fontSize: 13, color: 'rgba(255,255,255,0.35)', fontWeight: 600 }}>Updating…</span>
              </div>
            ) : (
              <p style={{ fontSize: 22, fontWeight: 900, color: '#fff', letterSpacing: -0.6, lineHeight: 1, animation: balStatus === 'updated' ? 'popIn 0.4s cubic-bezier(0.34,1.56,0.64,1)' : 'none' }}>
                {fmt(displayBalance)}
              </p>
            )}
          </div>
